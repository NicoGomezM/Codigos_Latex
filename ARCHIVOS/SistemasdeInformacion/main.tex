...
\begin{center}
    ...
    \begin{figure}[H]
        \centering
        \vspace*{\fill}
        \includegraphics[width=1\textwidth,height=0.5\textheight,keepaspectratio]{img/CurvaDeNolanActualizada.png}
        \vspace*{\fill}
    \end{figure}
\end{center}
...
% \subsection{\hl{Internet de las cosas}} TODO:
% \newpage
...
\begin{center}
    \begin{tikzpicture}
        \textbf{
        \node (A) at (0,0) {Definición del problema};
        \node (B) at (0,-1) {Estudio de factibilidad};
        \node (C) at (0,-2) {Análisis del sistema};
        \node (D) at (0,-3) {Diseño lógico};
        \node (E) at (0,-4) {Diseño físico};
        \node (F) at (0,-5) {Construcción};
        \node (G) at (0,-6) {Implementación};
        \node (H) at (0,-7) {Revisión post-implementación};
        \node (I) at (0,-8) {Mantenimiento};
        }
...
\begin{itemize}
    ...
    \end{itemize}
    \item \textbf{Construcción:} Punto en el cual se procede en la selección del lenguaje sobre el cual trabajaremos, el que utilizaremos para crear un programa y someterlo a pruebas de funcionamiento.
    ...
    \item \textbf{Mantenimiento:} Proceso de corrección de errores y mejoras del sistema. Existen 4 tipos de mantenimiento:
    \begin{itemize}
